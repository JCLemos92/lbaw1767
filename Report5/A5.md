...
| R04                | bannedMembers(__project_ID__ → projects NN, user_ID → users NN, role)                       |
| R05                | tasks(__task_ID__ UK NN, taskOwner → users, project_ID → projects NN, taskName NN, taskDescription NN, priority NN, dateCreated NN,dateLimit, status) |
| R06                | taskInProgress(__task_ID__ UK NN, taskWorker → users NN) |
| R07                | taskCompleted(__task_ID__ UK NN, taskWorker → users NN, completionDate NN) |
| R08                | forumPost(__forum_ID__ UK NN, project_ID → projects NN, owner_ID → users NN, forumTitle NN, post NN, dateCreated NN)                                |
| R09                | comments(__comment_ID__ UK NN, forum_ID → forum NN, owner_ID  → users NN, commentText NN, commentDate NN)                       |
| R10                | commentUpVote(__comment_ID__ → projects NN, voter_ID → users NN, role)                       |
| R11                | commentDownVote(__comment_ID__ → projects NN, voter_ID → users NN, role)                       |
| R12                | notification(__notification_ID__ UK NN, user_ID → users NN, sender_ID → users NN, project_ID → projects NN, task_ID, notificationText NN, dateSent NN) |
...
| **Table R04** (bannedMembers)                         |   |
| ----------------------------------------- | - |
| **Keys**: { project_ID }, { user_ID }              |   |
| **Functional Dependencies**               |   |
| FD0401          | { project_ID} → { user_ID } |
| FD0401          | { user_ID} → { project_ID } |
| **Normal Form** | BCNF                    |
...
| **Functional Dependencies**                      |   |
| FD0501          | { task_ID, project_ID } → {taskOwner, taskName, taskDescription, priority, dateCreated, dateLimit, status, taskWorker, completionDate} |
| **Normal Form** | BCNF                           |

| **Table R06** (taskInProgress)                                |   |
| ------------------------------------------------ | - |
| **Keys**: { task_ID }                         |   |
| **Functional Dependencies**                      |   |
| FD0601          | { task_ID } → {taskWorker} |
| **Normal Form** | BCNF                           |

| **Table R07** (taskCompleted)                                |   |
| ------------------------------------------------ | - |
| **Keys**: { task_ID }                         |   |
| **Functional Dependencies**                      |   |
| FD0701          | { task_ID } → {taskWorker, completionDate} |
| **Normal Form** | BCNF                           |
...
| **Functional Dependencies**               |   |
| FD0801          | { forum_ID, project_ID } → {owner_ID, forumTitle, post, dateCreated} |
| FD0802          | { owner_ID } → {forum_ID, project_ID, forumTitle, post, dateCreated} |
| **Normal Form** | BCNF                    |
...
| **Functional Dependencies**               |   |
| FD0901          | { comment_ID, forum_ID } → {owner_ID, commentText, commentDate} |
| FD0902          | { onwer_ID } → {comment_ID, forum_ID, commentText, commentDate} |
| **Normal Form** | BCNF                    |
 
| **Table R10** (commentUpVote)                         |   |
| ----------------------------------------- | - |
| **Keys**: { comment_ID, user_ID }              |   |
| **Functional Dependencies**               |   |
| FD1001          | { comment_ID } → { voter_ID } |
| FD1002          | { voter_ID } → { comment_ID } |
| **Normal Form** | BCNF                    |
 
| **Table R11** (commentDownVote)                         |   |
| ----------------------------------------- | - |
| **Keys**: { comment_ID, user_ID }              |   |
| **Functional Dependencies**               |   |
| FD1101          | { comment_ID } → { voter_ID } |
| FD1102          | { voter_ID } → { comment_ID } |
| **Normal Form** | BCNF                    |
...
| **Functional Dependencies**               |   |
| FD1201          | { notification_ID } → {user_ID, sender_ID, project_ID, task_ID, notificationText, dateSent} |
| FD1202          | { user_ID, sender_ID } → {notification_ID, project_ID, task_ID, notificationText, dateSent} |
| **Normal Form** | BCNF                    |
